...
<div class="px-[8%] flex gap-12 pt-10">
    <aside class="w-52 flex-shrink-0 font-42dot">
        ...
        <ul class="relative ml-[7px]">
            ...
            @php $categories = ['Tops' => '/shop', 'Bottom' => '/bottom', 'Dresses' => '/dresses', 'Outerwear' => '/outerwear', 'Activewear' => '/activewear']; @endphp
            ...
            <li class="relative py-[6px] pl-5">
                ...
                <a href="{{ $url }}" class="text-[13px] {{ request()->is(ltrim($url, '/')) ? 'font-bold text-black' : 'text-gray-500' }} hover:text-black transition-colors block">{{ $label }}</a>
            </li>
            @endforeach
        </ul>
    ...
    <main class="flex-1">
        <div class="flex justify-end mb-10">
            <form action="{{ url('/outerwear') }}" method="GET" class="relative w-72">
                <span class="absolute left-4 top-2.5">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                </span>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search" class="w-full bg-[#F3F4F6] border border-gray-200 rounded-full pl-12 pr-5 py-2.5 text-sm outline-none">
            </form>
        </div>
