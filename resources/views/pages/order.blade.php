...
<main class="max-w-6xl mx-auto mt-12 px-6 mb-12">
    @if(session('success'))
        <div class="mb-6 mx-2 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-sm font-medium">
            {{ session('success') }}
        </div>
    @endif
    ...
    <div class="flex gap-6">
        <aside class="w-1/4 bg-white rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] p-5 h-fit">
            <p class="font-bold text-sm mb-5 text-black">Welcome, {{ Auth::user()->name }}!</p>
                
            <div class="flex flex-col gap-2">
                <button class="flex justify-between items-center w-full py-3 px-4 border border-gray-200 rounded-lg text-sm font-semibold text-left bg-white text-black shadow-sm">
                    Orders 
                ...
                <form action="{{ route('logout') }}" method="POST" class="w-full">
                    @csrf
                    <button type="submit" class="flex justify-between items-center w-full py-3 px-4 text-sm font-normal text-black hover:bg-red-50 hover:text-red-600 rounded-lg transition text-left group">
                        Log Out 
                        <svg class="w-3 h-3 text-gray-400 group-hover:text-red-500 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <section class="w-3/4 bg-white rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] p-8 min-h-[480px] flex flex-col justify-between">
            <div>
                ...
                <div class="flex gap-2">
                    ...
                    <button class="border border-gray-300 px-5 py-1.5 rounded-full text-xs font-normal text-black hover:bg-gray-50"><a href="{{ route('order.done') }}">Done</a></button>
